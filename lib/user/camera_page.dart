...
class _CameraPageState extends State<CameraPage> {
  ...
  Future<void> _processPhotos() async {
    ...
    try {
      ...
      // Check if no diseases were detected
      final hasAnyDetectionsCamera = allResults.values.any((results) => results.isNotEmpty);
      
      if (!hasAnyDetectionsCamera) {
        // Show dialog with retake guidance
        await showDialog(
          context: context,
          barrierDismissible: false,
  ...
  Future<void> _selectFromGallery() async {
    ...
    if (images != null && images.isNotEmpty) {
      ...
      try {
        ...
        // Check if no diseases were detected
        final hasAnyDetectionsGallery = allResults.values.any((results) => results.isNotEmpty);
        
        if (!hasAnyDetectionsGallery) {
          // Show dialog with retake guidance
          await showDialog(
            context: context,
            barrierDismissible: false,
